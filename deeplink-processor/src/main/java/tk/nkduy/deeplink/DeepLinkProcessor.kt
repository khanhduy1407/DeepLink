...
import tk.nkduy.deeplink.ProcessorUtils.decapitalizeIfNotTwoFirstCharsUpperCase
import tk.nkduy.deeplink.ProcessorUtils.hasEmptyOrNullString
import tk.nkduy.deeplink.base.MatchIndex.ALLOWED_VALUES_DELIMITER
import tk.nkduy.deeplink.base.Utils
import tk.nkduy.deeplink.base.Utils.isConfigurablePathSegment
...
class DeepLinkProcessor(symbolProcessorEnvironment: SymbolProcessorEnvironment? = null) :
    ...
    private val deepLinkHandlerQName = "tk.nkduy.deeplink.handler.DeepLinkHandler"
    ...
    private fun verifyMethod(methodElement: XMethodElement) {
        ...
        // You can jut comment this check out for now if you need this to pass.
            if (methodElement.returnType.typeElement?.qualifiedName !in listOf(
                    ...
                    "tk.nkduy.deeplink.DeepLinkMethodResult"
                )
            ) {
                throw DeepLinkProcessorException(
                    element = methodElement,
                    message = (
                        "Only `Intent`, `androidx.core.app.TaskStackBuilder` or " +
                            "'tk.nkduy.deeplink.DeepLinkMethodResult' are supported. Please double " +
...
//    urisTrie.writeToOutoutStream(indexResource.openOutputStream());
    ...
    companion object {
        private const val PACKAGE_NAME = "tk.nkduy.deeplink"
