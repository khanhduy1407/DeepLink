...
```java
// Prefix all app deep link URIs with "app://nkduy"
...
@DeepLinkSpec(prefix = { "http{url_scheme_suffix}://nkduy.com")
...
```java
// Match all deeplinks which a scheme staring with "http".
@DeepLinkSpec(prefix = { "http{url_scheme_suffix(|s)}://{prefix(|www.)}nkduy.{domain(com|de)}")
@Retention(RetentionPolicy.CLASS)
public @interface WebDeepLink {
  String[] value();
}
...
// starts with .nkduy.com as well as ones that are only nkduy.com
@DeepLinkSpec(prefix = { "http{url_scheme_suffix}://{country_prefix}.nkduy.com",
 "http{url_scheme_suffix}://nkduy.com")
@Retention(RetentionPolicy.CLASS)
public @interface WebDeepLink {
  String[] value();
}
...
Add to your project `build.gradle` file (Latest version is [![](https://jitpack.io/v/tk.nkduy/DeepLink.svg)](https://jitpack.io/#tk.nkduy/DeepLink)
): 
```groovy
dependencies {
  implementation 'tk.nkduy:deeplink:x.x.x'
}
```
...
dependencies {
  implementation 'tk.nkduy:deeplink:x.x.x'
  ksp 'tk.nkduy:deeplink-processor:x.x.x'
}
```

**Note:** When using KSP (you have `ksp 'tk.nkduy:deeplinkdispatch-processor:x.x.x'` in your dependencies) at least one Kotlin source file *must* be present in the project or no output will be generated!
...
dependencies {
  implementation 'tk.nkduy:deeplink:x.x.x'
  kapt 'tk.nkduy:deeplink-processor:x.x.x'
}
```
...
dependencies {
  implementation 'tk.nkduy:deeplink:x.x.x'
  annotationProcessor 'tk.nkduy:deeplink-processor:x.x.x'
}
```
...
javaCompileOptions {
  annotationProcessorOptions {
    arguments = [
      ...
      'deepLink.customAnnotations': 'tk.nkduy.AppDeepLink|tk.nkduy.WebDeepLink'
    ]
  }
}
```
...
kapt {
  arguments {
    ...
    arg("deepLink.customAnnotations", "tk.nkduy.AppDeepLink|tk.nkduy.WebDeepLink")
  }
}
```
...
* Please refer to the [sample app](/sample/src/main/java/tk/nkduy/deeplinkdispatch/sample/DeepLinkActivity.java) for an example of how to use the library.
